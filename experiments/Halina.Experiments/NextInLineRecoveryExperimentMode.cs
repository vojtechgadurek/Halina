...
public sealed class NextInLineRecoveryConfig
{
    public string Path { get; set; } = "results_next_in_line";
    public int KmerLength { get; set; } = 31;
    public int StringCount { get; set; } = 400;
    public int KmersPerString { get; set; } = 25;
    public int BaseSeed { get; set; } = 123;
    public int ExperimentsPerStep { get; set; } = 100;
    public int PredictorSelectionDivisor { get; set; } = 3;
    public int TabuLimit { get; set; } = 3;
    public RangeConfig TableCount { get; set; } = new() { Start = 2, End = 3, Step = 1 };
    public DoubleRangeConfig MemoryMultiplier { get; set; } = new() { Start = 0.1, End = 1.2, Step = 0.01 };
    public bool SaveIndividualRuns { get; set; }
}

public sealed record NextInLineRecoveryPoint(
    int TableCount,
    double MemoryMultiplier,
    int RequestedExperiments,
    int SuccessfulExperiments,
    int FailedExperiments,
    double AverageRecoveryRatio,
    double VarianceRecoveryRatio,
    double AverageTrueRecoveredHashes,
    double AverageDecodedHashes,
    double AverageFalsePositiveHashes,
    double AverageOriginalHashes,
    double AverageDurationMs,
    double VarianceDurationMs);

public sealed record NextInLineRecoveryRunResult(
    int TableCount,
    double MemoryMultiplier,
    int ExperimentIndex,
    int Seed,
    int OriginalHashes,
    int TrueRecoveredHashes,
    int DecodedHashes,
    int FalsePositiveHashes,
    double RecoveryRatio,
    double DurationMs,
    bool Succeeded,
    string? Error = null);

public sealed record NextInLineRecoverySummary(
    string Version,
    NextInLineRecoveryConfig Config,
    IReadOnlyList<NextInLineRecoveryPoint> Points,
    IReadOnlyList<NextInLineRecoveryRunResult>? Runs);

public sealed class NextInLineRecoveryExperimentMode : IExperimentMode
{
    ...
    public string Name => "next-in-line";
    public string Usage => "next-in-line [config.json] [--parallel] [--max-concurrency N]";
    public string Description => "Sweep memory multipliers for a tabu-decoded 2/3-table 31-mer IBLT paired with NextInPathPredictor, measured over recovered ulong hashes.";
    ...
    public ExperimentRunSummary Run(ExperimentInvocation invocation)
    {
        ...
        var config = ExperimentModeSupport.LoadConfigOrCreateDefault<NextInLineRecoveryConfig>(
            ...
            "next_in_line_config_default.json",
            "Next-in-line");
        ...
        {
            Console.WriteLine("No next-in-line recovery specs defined.");
            return new ExperimentRunSummary(Name, 0, 0, 0, 0);
        }

        var runSpecs = BuildRunSpecs(pointSpecs, config).ToList();
        Console.WriteLine(
            $"Running {runSpecs.Count} next-in-line recovery experiments across {pointSpecs.Count} memory points " +
    ...
    private static NextInLineRecoveryRunResult ExecuteRun(RunSpec spec, NextInLineRecoveryConfig config)
    {
        ...
        try
        {
            ...
            var pipeline = new TablesBuilder<KmerData>()
                ...
                .Add(new NextInPathPredictor(spec.Seed, config.PredictorSelectionDivisor))
    ...
    private static void SaveSummaryArtifacts(NextInLineRecoverySummary summary, string outputDirectory)
    {
        ...
        string jsonPath = Path.Combine(outputDirectory, "next_in_line_recovery_summary.json");
        string csvPath = Path.Combine(outputDirectory, "next_in_line_recovery_summary.csv");
        ...
        Console.WriteLine($"Saved next-in-line recovery summary to {jsonPath}");
        Console.WriteLine($"Saved next-in-line recovery CSV to {csvPath}");
    }
